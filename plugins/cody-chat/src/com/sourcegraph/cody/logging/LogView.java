...
public class LogView extends ViewPart {
  ...
  @Override
	...
	private void addCopyAction() {
		var action = new Action("Copy") {
			...
			public void run() {
				...
				new Clipboard(Display.getDefault()).setContents(new Object[] {builder.toString()}, new Transfer[] {TextTransfer.getInstance()});
	...
	private void addClearAction() {
		...
		action.setImageDescriptor(PlatformUI.getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_ELCL_REMOVEALL));
		getViewSite().getActionBars().getToolBarManager().add(action);
	}
