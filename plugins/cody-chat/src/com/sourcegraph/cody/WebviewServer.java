...
public class WebviewServer {
    ...
    server.setHandler(
        new Handler.Abstract() {
          ...
          public boolean handle(Request request, Response response, Callback callback)
            ...
            System.out.println("request: " + request.getHttpURI() + " path: " + path);
  ...
  public int start() {
    try {
      this.server.start();
      var port = this.connector.getLocalPort();
      System.out.println(
          String.format("WebviewServer: listening to url http://localhost:%d", port));
      return port;
    } catch (Exception e) {
